...
export default class RelatorioConsumoPets extends Component<Props> {
    gerarDadosRelatorio(): ConsumoTipo[] {
        ...
        return Object.keys(agregado).sort().map(tipo => {
            let totalProdutosTipo = 0;
            let totalQtdProdutosTipo = 0;
            let totalServicosTipo = 0;
            let totalQtdServicosTipo = 0;

            const racasOrdenadas = Object.keys(agregado[tipo]).sort().map(raca => {
    ...
    render() {
        ...
        const dadosRelatorio = this.gerarDadosRelatorio();
